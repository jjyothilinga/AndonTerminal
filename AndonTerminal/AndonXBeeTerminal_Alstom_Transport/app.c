...
void APP_getOpenIssue(far OpenIssue *openIssue)
{
	...
	for(l=0; l < MAX_ISSUES; l++,i++)
	{
		...
		switch( ias.issues[i].data[k] )
		{
			...
			case '1': 
				strcpypgm2ram(&(openIssue->tag[j]), UI_MSG[	UI_MSG_EHS]);
			...
			case '3': 
				strcpypgm2ram(&(openIssue->tag[j]), UI_MSG[	UI_MSG_INDUSTRIALISATION]);
				length = strlen(&openIssue->tag[j]);
				j+=length;

				break;	
			case '4': 
				strcpypgm2ram(&(openIssue->tag[j]), UI_MSG[	UI_MSG_MANUFACTURING]);
				length = strlen(&openIssue->tag[j]);
				j+=length;
