...
void IAS_init(void)
{
	...
	ias.logonPassword[0] = '1';
	ias.logonPassword[1] = '3';
	ias.logonPassword[2] = '1';
	ias.logonPassword[3] = '3';
	ias.logonPassword[4] = '\0';
	...
	ias.logonStatus = Read_b_eep(LOGON_STATUS);
	Busy_eep();
	ClrWdt();
	
	if(ias.logonStatus == TRUE)
	{
		login();
...
void IAS_getOpenIssue(far OpenIssue *openIssue)
{
	...
	for(l=0; l < MAX_ISSUES; l++,i++)
	{
		...
		else if ( ias.issues[i].material == 2)
		{
			k++;
			openIssue->tag[j++] = 'S';openIssue->tag[j++] = 'A';openIssue->tag[j++] = 'F';openIssue->tag[j++] = 'E';openIssue->tag[j++] = 'T';openIssue->tag[j++] = 'Y';openIssue->tag[j++] = ':';
			strcpypgm2ram(&(openIssue->tag[j]), UI_CLASS_MSG[ias.issues[i].data[k] - '0']);
		...
		{
			strcpy(&(openIssue->tag[j]), ui_department[ias.issues[i].data[k] - '0']);
			length = strlen(&openIssue->tag[j]);
			j+=length;
...
{
	UINT8 i =openIssue->ID+1, j=0,k=0,l , m;
	UINT8 length;
	UINT8 temp[]="MATERIAL";
...
void IAS_raiseIssues(far UINT8* data , UINT8 material)
{
	UINT8 i,j,issueIndex;
	UINT8 *ptr;
	UINT8 tdata[MAX_KEYPAD_ENTRIES + 1] = {'\0'};
	UINT8 k;
	...
	for( i = 0 ; i < MAX_ISSUES ;i++)
	{
		if( ias.issues[i].state == ISSUE_RESOLVED )
		{
			...
			ias.issues[i].material = material;
			...
			if(ias.issues[i].material == FALSE)
			{
				k  = 0;
				while(ias.issues[i].data[k] != '\0')
				{
					tdata[k] = ias.issues[i].data[k];
					k++;
				}
				ias.issues[i].data[k] = tdata[k] = '0';
				k++;
				ias.issues[i].data[k] = tdata[k] = '\0';
				updateLog(tdata);
			}
