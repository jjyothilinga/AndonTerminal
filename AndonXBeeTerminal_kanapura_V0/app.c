...
typedef struct _APP
{
	...
	INT8 buzzerTimeout;
	UINT8 department[21];
	UINT8 department_raised[MAX_DEPARTMENTS + 1];
...
void APP_getOpenIssue(far OpenIssue *openIssue)
{
	...
	for(l=0; l < MAX_ISSUES; l++,i++)
	{
		...
		switch(ias.issues[i].data[k])
		{
			...
			case '1':
			openIssue->tag[j++] = 'Q';openIssue->tag[j++] = 'U';openIssue->tag[j++] = 'A';
			openIssue->tag[j++] = 'L';openIssue->tag[j++] = 'I';openIssue->tag[j++] = 'T';
			openIssue->tag[j++] = 'Y';openIssue->tag[j++] = ':';

			break;
			case '2':
			openIssue->tag[j++] = 'T';openIssue->tag[j++] = 'R';openIssue->tag[j++] = 'I';
			openIssue->tag[j++] = 'M';openIssue->tag[j++] = 'S';openIssue->tag[j++] = ':';
			break;

			case '3':
			openIssue->tag[j++] = 'C';openIssue->tag[j++] = 'U';openIssue->tag[j++] = 'T';
			openIssue->tag[j++] = 'T';openIssue->tag[j++] = 'I';openIssue->tag[j++] = 'N';
			openIssue->tag[j++] = 'G';openIssue->tag[j++] = ':';

			break;

			case '4':
			openIssue->tag[j++] = 'I';openIssue->tag[j++] = 'E';openIssue->tag[j++] = ':';

			break;

			case '5':
			openIssue->tag[j++] = 'B';openIssue->tag[j++] = 'E';openIssue->tag[j++] = 'D';
			openIssue->tag[j++] = ':';

			break;

			case '6':
			openIssue->tag[j++] = 'M';openIssue->tag[j++] = 'A';openIssue->tag[j++] = 'I';
			openIssue->tag[j++] = 'N';openIssue->tag[j++] = 'T';openIssue->tag[j++] = 'E';openIssue->tag[j++] = 'N';
			openIssue->tag[j++] = 'A';openIssue->tag[j++] = 'N';openIssue->tag[j++] = 'C';openIssue->tag[j++] = 'E';openIssue->tag[j++] = ':';
...
void APP_raiseIssues(far UINT8* data )
{
	...
	for( i = 0 ; i < MAX_ISSUES ;i++)
	{
		if( ias.issues[i].state == ISSUE_RESOLVED )
		{
			...
			ias.issues_raised++ ;
			department_type = ias.issues[i].data[2];
			ias.department_raised[department_type - '0']++;
			writeDeptRaise();
            if(ias.issues_critical == 0)
            {
...
UINT8 APP_comCallBack( far UINT8 *rxPacket, far UINT8* txCode,far UINT8** txPacket)
{
	...
	switch( rxCode )
	{
		...
		case CMD_UPDATE_DEPARTMENT:
			while(rxPacket[i] != '\0')
			{
				ias.department[i] = rxPacket[i+1];
				i++;
			}
			ias.department[i] = '\0';
			UI_updateDepartment(ias.department);
			length = 0;
			*txCode = CMD_UPDATE_DEPARTMENT;
			break;
