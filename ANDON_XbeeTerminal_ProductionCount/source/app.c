...
typedef struct _APP
{
	...
	UINT8 	openIssue;
	INT8 	buzzerTimeout;
	// Buffer used to hold the number of 16bits counts in data pack
...
static void handleMBwrite(void);
void storeCMDinBuffer(UINT8 *buffer, UINT8 command);
static void handleProductionCount(UINT8 *buffer);
...
//	if( LinearKeyPad_getPBState( PROXIMITY_SENSOR ) == FALSE )
	{
		ias.productionCount++;
		handleProductionCount(buffer);
		updateLog( buffer,CMD_PRODUCTION_COUNT ); 
