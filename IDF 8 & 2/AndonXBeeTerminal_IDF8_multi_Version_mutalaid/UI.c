...
UINT8 mapKey(UINT8 scancode, UINT8 duration)
{
	...
	switch(ui.state)
	{
		...
		keypressed = keyMap[scancode][0];
		 ...
		 if( ui.bufferIndex > 0 && ui.bufferIndex )
		{
		...
		{
			#ifdef MUTALIAID
			if( ( keypressed == '0') ||(keypressed == '\x0A')||(keypressed == '\x08') )
				keypressed = 0xFF;
			#else
			if( ( keypressed == '0') ||(keypressed == '\x0A')||(keypressed == '\x08')||(keypressed == '*') )
				keypressed = 0xFF;
			#endif
		...
		if( keypressed == '\x0C' || keypressed == '\x0B' || keypressed == '\x13' )
			keypressed = 0xFF;
...
*/
		ui.input[ui.inputIndex] = DEVICE_ADDRESS/10 + '0';
		ui.inputIndex++;
		ui.input[ui.inputIndex] = DEVICE_ADDRESS%10 + '0';
		ui.inputIndex++;
